...
class psychometrics:
    ...
    def perc_left_state(state, condition, data, left):
        frame = data[data['state'] == state]
        cond_frame = frame[frame['condition'] == condition]
        if left == 1:
            percent = len(cond_frame[cond_frame['choice'] == 0])/len(cond_frame)
        elif left == 0:
            percent = len(cond_frame[cond_frame['choice'] == 1])/len(cond_frame)
    ...
    def get_slope_measurement(y_data):
        # Calculate the slopes using the central difference method for the interior points
        # and forward/backward difference for the endpoints
        slopes = np.zeros(y_data.shape)
        slopes[1:-1] = (y_data[2:] - y_data[:-2]) / 2
        slopes[0] = y_data[1] - y_data[0]
        slopes[-1] = y_data[-1] - y_data[-2]

        # Generate corresponding x-values (assuming they are sequential and start from 1)
        x_data = np.arange(1, len(y_data) + 1)
        return slopes 
        
class plotting:
    def plot_psychometric_all_data(data, dpi):
        fig, axs = plt.subplots(1,1,figsize = (2.5,4), dpi=dpi)
